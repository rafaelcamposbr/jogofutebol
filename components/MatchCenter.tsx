...
export function MatchCenter({ initialView }: { initialView: any }) {
  ...
  return (
    <div className="match-center">
      ...
      <section className="player-ratings"><h2>Notas e desempenho</h2><div className="sports-table-wrap"><table className="sports-table"><thead><tr><th>Jogador</th><th>Pos.</th><th>Min.</th><th>Nota</th><th>G</th><th>A</th><th>Fin.</th><th>No alvo</th><th>Passes</th><th>Fadiga</th></tr></thead><tbody>{view.playerStats.filter((item: any) => item.team_side === "home").map((item: any) => <tr key={item.id}><td data-label="Jogador">{item.player_name}</td><td data-label="Posição">{item.position}</td><td data-label="Minutos">{item.minutes_played}</td><td data-label="Nota"><b>{Number(item.rating).toFixed(1)}</b></td><td data-label="Gols">{item.goals}</td><td data-label="Assistências">{item.assists}</td><td data-label="Finalizações">{item.shots}</td><td data-label="No alvo">{item.shots_on_target}</td><td data-label="Passes">{item.passes_completed}/{item.pass_attempts}</td><td data-label="Fadiga">{Math.round(item.extended_stats?.fatigue || 0)}</td></tr>)}</tbody></table></div></section>
