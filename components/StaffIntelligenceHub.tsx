...
function TutorialView({ data, busy, onAction }: { data: TutorialData | null; busy: boolean; onAction: (action: string) => void }) {
  ...
  return <section className="tutorial-center"><header><span className="tutorial-avatar">{data.advisor.initials}</span><div><h2>{data.advisor.name}</h2><p>{data.advisor.role} · {data.advisor.subtitle}</p></div></header><div className="tutorial-progress"><span style={{ width: `${Math.min(100, (progress.completed_steps.length + (progress.skipped_steps?.length || 0)) * 10)}%` }} /></div><strong>Status: {statusLabel(progress.status)}</strong><p>Etapa atual: {current?.title || "Tutorial concluido"}</p><ol>{data.steps.map((step) => <li className={statusClass(step.id)} key={step.id}>{step.id}. {step.title}</li>)}</ol>{current ? <a className="people-primary tutorial-center-link" href={current.route}>{current.actionLabel}</a> : null}<div className="tutorial-center-actions">{progress.status !== "active" ? <button className="people-primary" type="button" disabled={busy} onClick={() => onAction("reopen")}>Retomar tutorial</button> : <><button type="button" disabled={busy || progress.current_step === 1} onClick={() => onAction("back")}>Voltar</button><button type="button" disabled={busy} onClick={() => onAction("skip-step")}>Pular etapa</button><button type="button" disabled={busy} onClick={() => onAction("pause")}>Pausar</button><button type="button" disabled={busy} onClick={() => onAction("next")}>{progress.current_step === data.steps.length ? "Concluir" : "Continuar"}</button><button className="danger" type="button" disabled={busy} onClick={() => setConfirmEnd(true)}>Encerrar</button></>}</div>{confirmEnd ? <div className="tutorial-confirm" role="alertdialog" aria-label="Confirmar encerramento"><strong>Encerrar o tutorial?</strong><p>Voce podera reabri-lo depois em Minha Conta.</p><div><button type="button" disabled={busy} onClick={() => setConfirmEnd(false)}>Cancelar</button><button className="danger" type="button" disabled={busy} onClick={() => { setConfirmEnd(false); onAction("end"); }}>Confirmar encerramento</button></div></div> : null}</section>;
}
